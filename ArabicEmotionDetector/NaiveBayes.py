...
def naive_bayes(test_text):
    if os.path.isfile('NBmodel.joblib') and os.path.isfile('NBencoder.joblib') and os.path.isfile('NBcount.joblib'):
        loaded_model = load('NBmodel.joblib')
        loaded_count = load('NBcount.joblib')
        loaded_encoder = load('NBencoder.joblib')
        max_features = 200000
